...
| **D. 前端生成 OG PNG（指定 fallback）** | 前端 satori + canvas 渲染 PNG，再透過後端 `/og/upload`（#04 Storage → R2）儲存 | ✅ | ✅ | 改動大、字型/CJK 在瀏覽器同樣要處理；**spike 失敗時的優先退路** |
...
3. 產出結論並依下列決策樹分流：
   ...
   **決策樹（主人已確認）：**
   - **Workers Paid + resvg-wasm spike 通過** → 走方案 A，後端 Worker 生成 OG（進 #06b）。
   - **spike 失敗（bundle / CPU / 記憶體 / CJK 任一不可行）** → **優先 fallback：方案 D，
     改由前端 satori + canvas 生成 OG PNG，再 `POST /og/upload`（走 #04 Storage 寫進 R2
     `og/{draftId}.png`）儲存。** 後端僅保留 `/og/upload`，移除 `/og/preview` 的伺服器端
     渲染（改前端產生）。不採「OG 只在 self-host」，也不用易混淆的 `/upload/*`（已刪）。
