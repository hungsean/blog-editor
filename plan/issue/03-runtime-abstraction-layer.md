...
1. 新增 `src/app.ts`：只建立並 export Hono `app`（路由 mount），不綁特定 runtime。
   **保留現有 `app.route("/api", api)` 的 `/api` 前綴**（同 `index.ts:23`），`server.bun.ts`
   與 `worker.ts` 兩個入口都用這個 `app`，確保 Worker 仍接受 `/api/*`（#08 的 Pages 轉發不 rewrite path）。
...
4. route handler 依 #01 的 DB/repo 注入契約，**把 `c.var.db` 當參數傳進 repo 函數**
   （`getDraftById(c.var.db, id)`），不在 route 直接操作 drizzle，也不 import db 單例。
   middleware 設好的 `c.var.db` 即每 request 的正確 driver（self-host 單例 / Workers binding）。
...
- `github.ts:87` / `:139` 的 `Buffer.from` base64 **本 plan 不改寫成 Web API**，改由 #07 開
  `nodejs_compat` 來支援。故此處保留 `Buffer` 即可，不要為了 Worker 去動 GitHub 的 base64 邏輯。
...
- [ ] route handler 不再直接 import `db` / client 或讀 `process.env`；DB 一律經
      `c.var.db` 傳入 repo 函數（符合 #01 注入契約），其他設定走 `c.var.env`。
