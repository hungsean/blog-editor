...
- `server.bun.ts` — self-host（Bun）。開 `bun:sqlite` → migrate → 建 db 單例，讀一次 `process.env`，
  建 `S3Storage` 單例（aws-sdk），起 `Bun.serve`，啟動 `startPRChecker` 常駐輪詢。Bun-only / 常駐 /
  aws-sdk 的東西只能放這裡。
- `worker.ts` — Cloudflare Workers。每 request 從 `c.env.DB`（D1 binding）建 db、讀 `c.env`、用
  `c.env.BUCKET`（R2 binding）建 `R2Storage`。**mount Worker-safe 子集**：`drafts` / `slug` /
  `presets` / `images`（#04 起 `images` 已去 aws-sdk）。`og` 因 native（`@resvg/resvg-js`）相依，
  待 #06 清掉後才掛；完整啟動 + `nodejs_compat` 在 #07。
...
│   ├── storage/       # 物件儲存抽象（#04）：runtime-specific 實作，不可單檔同時 import 兩者
│   │   ├── types.ts   # Storage 介面 + StoredObject（純型別，無實作 import）
│   │   ├── s3.ts      # S3Storage：唯一 import @aws-sdk/client-s3，只由 server.bun.ts 鏈到
│   │   └── r2.ts      # R2Storage：用原生 R2 binding，只由 worker.ts 匯入（無 aws-sdk）
...
└── routes/
    ...
    ├── images.ts      # /images + /images/sync + /images/upload（圖片庫，走 c.var.storage）
    ├── og.ts          # /og/preview（native）+ /og/upload（走 c.var.storage）
    └── presets.ts     # translation presets CRUD
```

### 模組依賴關係

route 不再 import db 單例 / lib function / 物件儲存實作；DB 經 `c.var.db`、設定經 `c.var.env`、
物件儲存經 `c.var.storage`（#04，介面在 `lib/storage/types`）、外部服務經 factory（`createGithub` /
`createTranslator`）每 request 建。型別 `AppEnv` 由 `src/app.ts`（route）/ `src/runtime.ts`（worker）
提供，皆為 `import type`（erased，不進 runtime bundle）。

```
server.bun.ts → app.ts, lib/db.bun.ts, lib/db.ts, lib/env.ts, lib/github.ts, lib/prChecker.ts, lib/storage/s3.ts(aws-sdk)
worker.ts     → runtime.ts, lib/db.ts(createD1Db), lib/env.ts, lib/storage/r2.ts(R2 binding), routes/{drafts,slug,presets,images}
app.ts        → runtime.ts, routes/api.ts（→ 全部 route）
runtime.ts    → hono/cors, type DrizzleDB, type Env, type Storage（無路由、無 Bun-only、無實作）
...
images.ts  → c.var.db→repos/images, c.var.storage
og.ts      → c.var.storage, lib/ogImage（native，self-host only 直到 #06）
presets.ts / slug.ts → c.var.db→repos/*
prChecker.ts → 注入 db / github client（不 import db 單例 / lib function）
lib/github|translator.ts → lib/env.ts（型別）；module load 不讀 process.env
lib/storage/s3.ts → @aws-sdk/client-s3 + type R2Env；lib/storage/r2.ts → @cloudflare/workers-types（型別）
...
- **env provider（#03）**：`lib/env.ts` 的 `readEnv(source)` 是唯一的環境變數讀取點（純函數），
  把 `process.env` / `c.env` 解析成 `Env`。**消除「import 即讀 env」**——`github` / `translator`
  改成 `createX(env)` factory（module load 不再讀 `process.env`），Workers 上 binding 才來得及注入。
- **factory 而非單例（#03）**：外部服務 client（GitHub / translator）每 request 由 `c.var.env`
...
- **物件儲存抽象（#04）**：`lib/storage/types.ts` 只放 `Storage` 介面（`isEnabled` / `put` / `get` /
  `list` / `delete` / `publicUrl`）+ `StoredObject`，**無任何實作 import**。兩個 runtime-specific 實作各自
  一支檔：`s3.ts`（`S3Storage`，**唯一** import `@aws-sdk/client-s3`，只由 `server.bun.ts` 鏈到）、
  `r2.ts`（`R2Storage`，原生 R2 binding，只由 `worker.ts` 匯入）。由 `makeStorage` provider 每 request
  注入 `c.var.storage`，route 只認介面、不 import 任一實作。**嚴禁** barrel `storage/index.ts` 同時靜態
  import 兩支實作（會把 aws-sdk 拉進 Worker bundle）。`images` / `og` 改走 `c.var.storage`；廢棄的
  `/upload/*`（含 `data/og-temp/` 的 `node:fs` 暫存）已整個刪除。
- **Workers bundle 純淨度（#03 / #04）**：`runtime.ts` 與 route 的 `AppEnv` 一律 `import type`（erased）；
  `worker.ts` 不 import `app.ts`（否則連帶 `routes/api.ts` → `og` 的 `@resvg/resvg-js` native 相依會進
  bundle）。#04 起 `images` 已去 aws-sdk 可掛進 worker；aws-sdk 只進 self-host（Bun）產物，以
  `wrangler deploy --dry-run --outdir` + grep bundle 驗收。`github.ts` 的 `Buffer` base64 留待 #07 開
  `nodejs_compat`。
