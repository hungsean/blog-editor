...
| **D. 前端生成 OG PNG（指定 fallback）** | 前端 satori + canvas 渲染 PNG，再透過後端既有 upload/storage endpoint 存進 R2 | ✅ | ✅ | 改動大、字型/CJK 在瀏覽器同樣要處理；**spike 失敗時的優先退路** |
...
3. 產出結論並依下列決策樹分流：
   ...
   **決策樹（主人已確認）：**
   ...
   - **spike 失敗（bundle / CPU / 記憶體 / CJK 任一不可行）** → **優先 fallback：方案 D，
     改由前端 satori + canvas 生成 OG PNG，再透過後端既有 upload/storage endpoint
     （#04 的 Storage）存進 R2。** 不採「OG 只在 self-host」。
